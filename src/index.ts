    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        const lines = [
          ...
          `  --- 8 Trust Signals ---`,
          `  success_rate:      ${successRate !== null ? successRate.toFixed(3) : "--"} (w=0.20)`,
          `  tool_failure_rate: ${toolFailureRate !== null ? toolFailureRate.toFixed(3) : "--"} (w=0.15, lower=better)`,
          `  avg_severity_inv:  ${sevStats.avgSeverity !== undefined ? (1 - sevStats.avgSeverity/1000).toFixed(3) : "--"} (w=0.15, inverted)`,
          `  critical_hit_rate: ${metrics.cycles.length > 0 ? (metrics.trustSignals.criticalHits / metrics.cycles.length).toFixed(3) : "0.000"} (w=0.25)`,
          `  approval_passed:   ${(metrics.trustSignals.approvalPasses / 20).toFixed(3)} (w=0.10, capped at 20)`,
          `  approval_denied:    ${(metrics.trustSignals.approvalDenials / 20).toFixed(3)} (w=0.10, negative)`,
          `  user_nudge:        ${(metrics.trustSignals.userNudges / 10).toFixed(3)} (w=0.20, negative)`,
          `  fast_lane_use:     ${(metrics.trustSignals.fastLaneUses / 10).toFixed(3)} (w=0.05)`,
