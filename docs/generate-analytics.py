...
function render() {{
  ...
  document.getElementById('tbody').innerHTML = data.slice(0,200).map(r=>{
    const ts = r.timestamp ? new Date(r.timestamp).replaceTime ? r.timestamp.slice(0,16).replace('T',' ') : r.timestamp.slice(0,16).replace('T',' ') : '';
    const dots = `<span class="dot dot-${r.result}"></span>`;
    const tags = (r.patterns||[]).map(p=>`<span class="tag tag-pat">${{p.slice(0,20)}}</span>`).join('');
    const cmd = (r.command||'').slice(0,70);
    return `<tr><td>${{ts}}</td><td>${{(r.sessionId||'').split(':')[0]}}</td><td>${{dots}}${{r.result||''}}</td><td>${{tags}}</td><td class="mono" title="${{r.command||''}}">${{cmd}}</td></tr>`;
  }).join('');
}}
